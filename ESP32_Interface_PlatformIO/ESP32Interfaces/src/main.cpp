...
void loop()
{
  if (Serial.available() > 0)
  {
    ...
    std::copy(data.begin(), data.end(), std::begin(blockcontent));

    // Write the data to the block
    error_status = writeBlock(block, blockcontent);
    // Serial.print("Error status: ");
    // Serial.println(error_status);

    if (error_status == MFRC522::STATUS_OK) {
      Serial.write("1"); // Send success status (1) to PC
    }
    else {
      Serial.write("2"); // Send failure status (2) to PC
    }
